...
module.exports = {
    ...
    getBookById: async (req, res, next) => {
        console.log('get Book by ID function called', req.body);
    ...
    setRating: async (req, res, next) => {
        ...
        await Book.findOne({id: bookID}, async function (err, foundBook) {
            ...
            if (foundBook) {
                console.log('book found');
                const authorIDFound = foundBook.rating.listOfVotes.find(e => e.idsOfVoter === authorID);
                ...
                if (!authorIDFound) {
                    ...
                    User.findOne({id: authorID}, async function (err, foundUser) {
                        if (foundUser) {
                            const bookIDFound = foundUser.myRatings.find(e => e.idOfBook === bookID);
                            if (!bookIDFound) {
                                console.log('book wasn\'t yet rated');
                                foundUser.myRatings.push({idOfBook: bookID, rating});
                                foundUser.save(function (error) {
                                    if (error) {
                                        console.log(error);
                                    } else {
                                        console.log('save rating successfully in user');
                                    }
                                });
                            } else {
                                console.log('user already rated that book');
                            }
                        } else {
                    ...
                    console.log('voter found', authorIDFound);
